...
public class VasarloDao {
     ...
     * A naplózó példánya.
     */
	private final static Logger logger = LoggerFactory.getLogger(VasarloDao.class);
	...
	public List<Vasarlo> lekerdezVasarlok() {
		...
		try {
			...
			logger.error("A vásárló(k) lekérdezése nem sikerült! " + e.getMessage());
		}
	...
	public void hozzaAdVasarlot(Vasarlo vasarlo){
		try {
			...
			logger.error("A vásárló hozzáadása nem sikerült! " + e.getMessage());
		}
	}
