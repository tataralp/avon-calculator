...
public class RendelesFelvetelKepernyoController {
     ...
     * A naplózó példánya.
     */
	private final static Logger logger = LoggerFactory.getLogger(RendelesFelvetelKepernyoController.class);
	...
	public void initialize() {
		...
		logger.info("A rendelés felvétel képernyő inicializálása megtörtént.");
	}
	...
	public void menteskattraTermek() {
		...
		try {
			...
			logger.warn("A kedvezmény nem számformátum.");
			kedvezmeny = -1;
		}
		try {
			cikkszam = Integer.parseInt(cikkszFelvKitolt.getText());
		} catch (NumberFormatException e) {
			logger.warn("A cikkszám nem számformátum.");
			cikkszam = -1;
		}
		try {
			darabszam = Integer.parseInt(dbszFelvKitolt.getText());
		} catch (NumberFormatException e) {
			logger.warn("A darabszám nem számformátum.");
			darabszam = -1;
		}
		try {
			ar = Integer.parseInt(arFelvKitolt.getText());
		} catch (NumberFormatException e) {
			logger.warn("Az ár nem számformátum.");
			ar = -1;
		}
		...
		logger.info("A termék mentésre került.");
	}
	...
	public void menteskattrarend() {
		...
		logger.info("A rendelés mentésre került.");
	}
